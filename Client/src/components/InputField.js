...
const InputField = ({ value, setValue, nameKey, type, invalidFields, setInvalidField }) => {
    return (
        <div className='w-full flex flex-col relative mb-2'>
            {value.trim() !== '' && <label className='text-[10px] absolute top-0 left-[12px] text-sm block bg-white px-1' htmlFor={nameKey}>{nameKey?.slice(0, 1).toUpperCase() + nameKey?.slice(1)}</label>}
            <input
                type={type || 'text'}
                className='px-4 py-2 rounded-sm border w-full my-3 mt-2'
